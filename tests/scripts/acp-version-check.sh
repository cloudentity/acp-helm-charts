...
ACP_VERSION="$(yq eval '(.version)' ./charts/acp/Chart.yaml)"
ACP_VERSION_IN_KUBE_ACP_STACK="$(yq eval '(.dependencies[]|select(.name == "acp").version)' ./charts/kube-acp-stack/Chart.yaml)"
